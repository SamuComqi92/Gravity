...
class Cluster_collision : 
	...
	"""
	def __init__(self, Number_obj, Epochs, Radius, Mass_obj = 1e33, Pause = 0.01, flag_mass = True, flag_rot = False, flag_size = False) :
		"""
		...
		- flag_size			If True, the two clusters have different sizes (proportion 1:2)
		"""
		...
		#Random position (in meters) of the objects in two spherical regions
		t1 = np.random.uniform(0,1, size = self.Number_obj)
		u1 = np.random.uniform(0,1, size = self.Number_obj)
		v1 = np.random.uniform(0,1, size = self.Number_obj)
		x1 = self.Radius*np.sqrt(t1)*np.cos(2*np.pi*u1)*np.sin(np.pi*v1)
		y1 = self.Radius*np.sqrt(t1)*np.sin(2*np.pi*u1)*np.sin(np.pi*v1)
		z1 = self.Radius*np.sqrt(t1)*np.cos(np.pi*v1)
		self.Pos_planets1 = self.AU*np.vstack([x1,y1,z1]).T
		if flag_size == True :
			t2 = np.random.uniform(0,2, size = self.Number_obj)
			u2 = np.random.uniform(0,2, size = self.Number_obj)
			v2 = np.random.uniform(0,2, size = self.Number_obj)
			x2 = self.Radius*np.sqrt(t2)*np.cos(2*np.pi*u2)*np.sin(np.pi*v2) + 3*self.Radius
			y2 = self.Radius*np.sqrt(t2)*np.sin(2*np.pi*u2)*np.sin(np.pi*v2)
			z2 = self.Radius*np.sqrt(t2)*np.cos(np.pi*v2)
			self.Pos_planets2 = self.AU*np.vstack([x2,y2,z2]).T
		else :
			t2 = np.random.uniform(0,1, size = self.Number_obj)
			u2 = np.random.uniform(0,1, size = self.Number_obj)S
			v2 = np.random.uniform(0,1, size = self.Number_obj)
			x2 = self.Radius*np.sqrt(t2)*np.cos(2*np.pi*u2)*np.sin(np.pi*v2) + 2*self.Radius
			y2 = self.Radius*np.sqrt(t2)*np.sin(2*np.pi*u2)*np.sin(np.pi*v2)
			z2 = self.Radius*np.sqrt(t2)*np.cos(np.pi*v2)
			self.Pos_planets2 = self.AU*np.vstack([x2,y2,z2]).T
		...
		if self.flag_mass == True :
			self.Mass_tot1 = self.Mass_obj*np.random.randint(1,4,size=(self.Number_obj,1))
			self.Mass_tot2 = self.Mass_obj*np.random.randint(1,4,size=(self.Number_obj,1))
		else :
			self.Mass_tot1 = self.Mass_obj*np.ones((self.Number_obj,1))
			self.Mass_tot2 = self.Mass_obj*np.ones((self.Number_obj,1))
		
		#Random velocities (in km/s) of the objects in the cluster (components along the x,y, and z axis)
		Vel1 = np.array([5000000,0,0])
		Vel2 = np.array([-5000000,0,0])
		Velocities1 = []
		Velocities2 = []
		for i in range(self.Number_obj) :
			Velocities1.append(Vel1)
			Velocities2.append(Vel2)
		self.Velocities1 = np.array(Velocities1)
		self.Velocities2 = np.array(Velocities2)
	
		#Combina data
		self.Pos_planets = np.vstack([self.Pos_planets1,self.Pos_planets2])
		self.Mass_tot = np.vstack([self.Mass_tot1,self.Mass_tot2])
		self.Velocities = np.vstack([self.Velocities1,self.Velocities2])
	...
	def acceleration(A, B, alpha1, alpha2, aa) :
		...
		for i in range(len(A)) :
			a = Cluster_collision.Sign(A[i]-B[i]) * aa[i]*np.array([np.cos(alpha1[i])*np.cos(alpha2[i]),np.cos(alpha1[i])*np.sin(alpha2[i]), np.sin(alpha1[i])])
			Final_a.append(a)
	...
	def Draw(self) :
		for t in range(self.Epochs) :
			...
			self.Velocities = self.Velocities + Cluster_collision.acceleration(self.Pos_planets, Pos_CM, alpha1, alpha2, Accelerations)*self.Delta_t
			...
			#Creating the plot
			Xlim_l = -3.*self.Radius*self.AU
			Xlim_u = 4.*self.Radius*self.AU
			Ylim_l = -3.*self.Radius*self.AU
			Ylim_u = 4.*self.Radius*self.AU
			Zlim_l = -3.*self.Radius*self.AU
			Zlim_u = 4.*self.Radius*self.AU
			#Plot
			fig = plt.figure(1)
			fig.set_facecolor('black')
			ax = fig.add_subplot(111, projection='3d')
			if self.flag_rot == True :
				ax.view_init(20,50+0.5*t)
			else :
				ax.view_init(20,60)
